...
class refApp(tk.Tk):
    def __init__(self, size):
        ...
        self.bg_image = ImageTk.PhotoImage(Image.open(current_path + "/img/bg_gradient.jpg").resize((1920, 1080)))

        #logo with name
        self.logo_image = ImageTk.PhotoImage(Image.open(current_path + "/img/trial logo 2.png").resize((256, 406)))

        #logo w/out name
        self.logo_image_3 = ImageTk.PhotoImage(Image.open(current_path + "/img/trial logo.png").resize((70, 70)))


        self.assessment_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/assessment.png")).resize((40, 40)))
        self.records_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/verify.png")).resize((40, 40)))
        self.account_user_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/users.png")).resize((30, 30)))
        self.register_patient_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/patient.png")).resize((40, 40)))
        self.log_out_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/logout.png")).resize((30, 30)))
        self.about_us_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/info.png")).resize((30, 30)))
        self.patient_ID_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/patient-id.png")).resize((40, 40)))
        self.search_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/search-light.png")).resize((20, 20)))
        self.foot_icon = ImageTk.PhotoImage(Image.open(os.path.join(current_path + "/img/templates.png")).resize((220, 220)))
        ...
        self.mmsu_logo = ImageTk.PhotoImage(Image.open(os.path.join(current_path+ "/img/mmsu-logo.png")).resize((80,80)))
        self.cpe_logo = ImageTk.PhotoImage(Image.open(os.path.join(current_path+ "/img/cpe-logo.png")).resize((80,80)))
    ...
    def styles(self):
        ...
        self.style.configure('main.TButton', font = ("Raleway", 16, "bold"), padding = [10, 5, 10, 5])
        self.style.configure('nav.TButton', font = ("Raleway", 16, "bold"), padding = [12, 8, 12, 8])
...
class Title(ttk.Frame):
    def __init__(self, parent, style):
        ...
        self.school_logo_frame.grid(row = 0, column = 0, padx = 60, pady = 20)
        ...
        self.logo_image_label.grid(row=1, column=0, padx=60, pady=(15, 15))
        self.login_label = ttk.Label(self.login_frame, text="Login to your Accounts", font= ( "Raleway", 18,"bold"), bootstyle= 'inverse-light')
        self.login_label.grid(row=2, column=0, padx=60, pady=(15, 15))
        self.pin_entry = ttk.Entry(self.login_frame, font= ('montserrat', 12, 'bold'), bootstyle = 'default', show= '*')
        self.pin_entry.grid(row = 3, column = 0, padx=60, pady=(15, 15))
        self.login_button = ttk.Button(self.login_frame, text="Login", command= self.login, 
                                       bootstyle = 'primary', cursor = 'hand2', width = 13, style= 'main.TButton', takefocus= False)
        self.login_button.grid(row=5, column=0, padx=60, pady=(10, 40))
...
class MenuBar(ttk.Frame):
    def __init__(self, parent, style):
        ...
        self.navigation_frame_label = ttk.Label(self.navigation_frame, text= "Gait Insight", image=self.master.logo_image_3, bootstyle = 'inverse-light',
                                                             compound="left", font= ("Raleway", 24, "bold"), takefocus= False)
    ...
    def StartAssessment(self):
        ...
        self.title_label = ttk.Label(self.StartAssessment_frame, text = "Gait Assessment Section", font= ("Raleway", 24, 'bold'), bootstyle = "inverse-light")
        self.title_label.grid(row = 0, column = 0, pady=(30,30))
        ...
        self.insole_status = ttk.Label(self.insole_syncing_frame, text= "Pressure Insole Sensor", font=('Raleway', 18, 'bold'))
        self.insole_status.grid(row = 0, column = 0, padx = 20, pady=(30,40))
        ...
        self.patient_label = ttk.Label(self.patient_number, text= "Select Current Patient", font=('Raleway', 18, 'bold'))
        self.patient_label.grid(row = 0, column = 0, pady=(30,40))
    ...
    def about_us(self):
        ...
        self.about_label.grid(row = 0, column = 0, pady=(20,10))
        ...
        self.paragraph_label = ttk.Label(self.paragraph_frame, text= "Welcome to Gait Insight, the premier tool for healthcare practitioners conducting gait assessments. \nOur application is meticulously designed to provide comprehensive and invaluable information about gait\npatterns,empowering healthcare professionals to conduct thorough evaluations and deliver personalized\ncare to their patients."
                                         , font=('raleway', 12), bootstyle = 'inverse-secondary')
        ...
        #research adviser
        self.research_ad = ttk.Label(self.picture_frame, text="", image= self.master.dale, bootstyle = 'inverse-light')
        ...
        self.research_ad_mail = ttk.Label(self.picture_frame, text="dummyAccount@gmail.com", bootstyle = 'inverse-light', font=('montserrat', 8, 'bold'))
        self.research_ad_mail.grid(row = 2 , column = 0, pady=(3,10))
        ...
        self.dale_p_text .grid(row = 2, column = 0, padx = 5, pady=(5,10))
        ...
        self.john_p_text .grid(row = 2, column = 0, padx = 5, pady=(5,10))
        ...
        self.yan_p_text .grid(row = 2, column = 0, padx = 5, pady=(5,10))
        ...
        self.ay_ay_text .grid(row = 2, column = 0, padx = 5, pady=(5,10))
        ...
        self.salmo_text .grid(row = 2, column = 0, padx = 5, pady=(5,10))
    ...
    def reg_Patients(self):
        ...
        self.reg_label = ttk.Label(self.regpatient_frame, text = "Register Patient Section", font= ("Raleway", 24, 'bold'), bootstyle = "inverse-light")
        self.reg_label.grid(row = 0, column = 0, pady=(30,30))
        ...
        self.search_entry = ttk.Entry(self.search_bar_frame, font= ('montserrat', 14, 'bold'), bootstyle = 'default', width = 30)
    ...
    def Patients_records(self):
        ...
        self.records_label = ttk.Label(self.patient_records_frame, text = "Patient Records Section", font= ("Raleway", 24, 'bold'), bootstyle = "inverse-light")
        self.records_label.grid(row = 0, column = 0, pady=(30,30))
        ...
        self.search_entry_2 = ttk.Entry(self.search_bar_2_frame, font= ('montserrat', 14, 'bold'), bootstyle = 'default', width = 30)
        ...
        self.scroll_bar_container.grid(row = 2, column = 0, sticky= "nsew", padx = 20, pady = 20)
        self.scroll_bar_container.grid_columnconfigure(0, weight = 1)
        self.scroll_bar_container.grid_rowconfigure(0, weight = 1)

        self.scrollable_frame = ScrolledFrame(self.scroll_bar_container, autohide= False, bootstyle = 'light-rounded', height = 600)
    ...
    def display_assessment_record(self, row_index, client_id, date_time, assessment_num, first_name, last_name):
        # Create box frame
                ...
                self.assessment_title = ttk.Label(self.box_frame, text=f"Assessment {assessment_num} - {date_time} - {client_id}", font = ('Montserrat', 14, 'bold'),
...
class Side_Cam(ttk.Frame):
    def __init__(self, parent, style):
        ...
        self.container_frame.grid(row = 0, column = 0, padx = 20, pady = 20, sticky= "nsew")
        ...
        self.text_frame.grid(row = 0, column = 0, pady = 20, padx = 20)

        #create a frame for web cam
        self.web_cam_frame = ttk.Frame(self.container_frame, borderwidth=0, bootstyle = "light")
        self.web_cam_frame.grid(row = 1, column = 0, pady = 10, padx = 10)
        ...
        #choose side button
        self.right_button = ttk.Button(self.choose_buttons_frame, text= "START RIGHT LEG", width = 18, style = 'main.TButton', takefocus= False,
    ...
    def change_button(self, state):
        ...
        if state == 'right':
            ...
            self.record_label = ttk.Label(self.record_frame, text= "Press Space to Start Recording", font = ("montserrat", 18, 'bold'), bootstyle = "inverse-primary")
            self.record_label.grid(row = 0, column = 0, pady = 20)
        ...
        if state == 'left':
            ...
            if self.master.side_state['Right'] == 1 and self.master.side_state['Left'] == 0:
                ...
                self.back_button = ttk.Button(self.choose_buttons_frame_2, text= "GO BACK", width = 18, style = 'danger.TButton', takefocus= False,
                                          cursor = "hand2", command= lambda:self.master.change_frame(self, MenuBar))
                self.back_button.grid(row = 0, column = 2, padx = (20, 0))
                ...
                self.end_button = ttk.Button(self.choose_buttons_frame_2, text= "END VIDEO TAKING", width = 18, style = 'main.TButton', takefocus= False,
                                          cursor = "hand2", command= lambda:self.master.change_frame(self, Process_Table))
                self.end_button.grid(row = 0, column = 1, padx = (20, 20))
...
class Process_Table(ttk.Frame):
    ...
    def table_frame(self):
        ...
        if self.master.side_state['Right'] == 1 and self.master.side_state['Left'] == 0:
            ...
            self.side_selector = ttk.Combobox(self.tool_frame_2, textvariable=self.side_var, values=['Right'], state="readonly", width= 19)

        elif self.master.side_state['Right'] == 0 and self.master.side_state['Left'] == 1:
            # Create a Combobox to select the side (Left or Right)
            self.side_var = tk.StringVar()
            self.side_var.set('Left')  # Default value
            self.side_selector = ttk.Combobox(self.tool_frame_2, textvariable=self.side_var, values=['Left'], state="readonly",width= 19)
        else:
            # Create a Combobox to select the side (Left or Right)
            self.side_var = tk.StringVar()
            self.side_var.set('Left')  # Default value
            self.side_selector = ttk.Combobox(self.tool_frame_2, textvariable=self.side_var, values=['Left', 'Right'], state="readonly", width= 19)
