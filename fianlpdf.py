...
def generate_pdf(client_id, consultation_date, assessment_num, phase_data_dict, landscape_mode=False):
    try:
        # Generate PDF
        orientation = landscape(letter) if landscape_mode else letter  # Change legal to letter
        pdf_filename = f"{client_id}_{consultation_date}_{assessment_num}_data.pdf"
        pdf_path = os.path.join("/home/silog/Thesis_Source_Codes/PDF DATAS", pdf_filename)  # Combine path with filename
        doc = SimpleDocTemplate(pdf_path, pagesize=orientation, leftMargin=20, rightMargin=20, topMargin=20, bottomMargin=20)

        # Create Paragraph objects for client_id and assessment_num
        styles = getSampleStyleSheet()
        client_id_text = Paragraph(f"<b>Client ID:</b> {client_id}", styles['Normal'])
        ...
        for phase, (table_data, columns) in phase_data_dict['Right'].items():
            ...
            columns_without_side_phase = [col for col in columns if col not in ['Side', 'Phase']]
            ...
            for row in table_data:
                ...
                for index, cell in enumerate(row):
                    if columns[index] not in ['Side', 'Phase']:  # Exclude 'Side' and 'Phase' columns
                        if columns[index] == 'Image':
                            ...
                            image_path = f"/home/silog/Thesis_Source_Codes/Data Inputs/insole_rep/{side}/{cell}.png"  # Adjust the path as per your directory structure
                            table_row.append(Image(image_path, width=100, height=100))
                        else:
        ...
        messagebox.showinfo("PDF Created", f"PDF '{pdf_path}' has been successfully created!")
...
def fetch_phase_data(client_id, consultation_date, assessment_num):
    try:
        ...
        # Initialize a dictionary to store table data for each phase
        ...
        for phase in phases:
            ...
            # Determine side (Right or Left) based on the 'side' column in the data
            side = data[0][-1]  # Assuming the side is consistent within each phase

            # Store table data for the current phase and side
            phase_data_dict[side][phase] = (data, ['Image', 'Frame', 'Hips', 'Knees', 'Ankle', 'Insole', 'Side'])
